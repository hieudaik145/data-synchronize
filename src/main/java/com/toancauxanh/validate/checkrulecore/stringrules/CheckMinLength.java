...
public class CheckMinLength implements CheckRuleCore {

    @Override
    public MethodSpec generateMethodSpec(Standard standard) {

        int minLength = (int) standard.getStandardDetails().get(0).getAttributeValue();

        MethodSpec methodSpec = MethodSpec.methodBuilder("isValidate").addModifiers(Modifier.PRIVATE)
                .returns(boolean.class).addParameter(Object.class, "valueCheck")
                .beginControlFlow("try")
                .addStatement("String  value = (String) valueCheck")
                .addStatement("return value.length() >= " + minLength).nextControlFlow("catch ($T e)", Exception.class)
